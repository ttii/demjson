...
class helpers(object):
    ...
    def safe_chr(codepoint):
        """Just like Python's chr() but works in narrow-Unicode Pythons."""
    ...
    def strip_format_control_chars(txt):
        ...
        * http://en.wikipedia.org/wiki/Unicode_control_characters
        ...
        """
        import unicodedata
        txt2 = filter(lambda c: unicodedata.category(str(c)) != 'Cf', txt)
    ...
    def surrogate_pair_as_unicode(c1, c2):
        """Takes a pair of str surrogates and returns the equivalent str character.
        ...
        v = (a << 10) | b
        v += 0x10000
        return helpers.safe_chr(v)

    @staticmethod
    def unicode_as_surrogate_pair(c):
        """Takes a single str character and returns a sequence of surrogate pairs.

        The output of this function is a tuple consisting of one or two str
...
class buffered_stream(object):
    ...
    def skipuntil(self, testfn):
        ...
        The provided test function should take a single str
...
class json_options(object):
    ...
    def __init__(self, **kwargs):
        ...
        will be represented as a str escape sequence; if False then
        the actual real str character will be inserted if possible.

        The 'escape_unicode' can also be a function, which when called
        with a single argument of a str character will return True
...
class JSON(object):
    ...
    def decode_string(self, state):
        ...
        Takes a string and a starting index, and returns a Python
        string (or str string) and the index of the next unparsed
        character.
            ...
            if c == quote:
                ...
                if not c:
                    ...
                    if high_surrogate:
                        ...
                        state.push_error('Low str surrogate must be proceeded by a high surrogate', position = escape_position,
                                         outer_position = string_position,
                                         context = 'String')
                        should_stop = state.should_stop
                        _append(u'\ufffd')  # replacement char
    ...
    def encode_string(self, s, state):
        ...
        # convert Python2 'str' types to str strings first.
        ...
        while i < imax:
            ...
            if cord < 256 and asciiencodable[cord] and isinstance(encunicode, bool) \
                ...
                # Always str escape ASCII-control characters
    ...
    def skipws(self, state):
        """Skips all whitespace, including comments and str whitespace
    ...
    def __sanity_check_start(self, state):
        ...
        as it's first character any str character.  Thus the first
        ...
        if len(unitxt) >= 2:
            ...
            if first in self._string_quotes:
                ...
                if ((ord(first) < 0x20 or ord(first) > 0x7f) or \
                    ...
                    # Found non-printable ascii, must check str
    ...
    * 'indent_limit'    (Integer or None, default None)
    ...
    in string literals must be \u escaped.  Should be one of:
        ...
        * function -- A user-supplied function that accepts a single
             str character and returns True or False; where True
...
def decode(txt, encoding = None, **kwargs):
    ...
    * 'allow_xxx', 'warn_xxx', and 'forbid_xxx'    (Booleans)
    ...
    Unicode decoding:
    -----------------
    The input string can be either a python string or a python str
    string (or a byte array in Python 3).  If it is already a str
